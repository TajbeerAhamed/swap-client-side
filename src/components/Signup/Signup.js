...
const Signup = () => {
  ...
  const saveUser = (role, email) => {
    const usersSaved = { role, email };
    console.log(usersSaved);
    fetch("https://swap-server-site.vercel.app/usersList", {
      method: "POST",
      headers: {
      ...
      <div className="w-full mx-auto max-w-md p-4 rounded-md shadow sm:p-8 bg-white-400 text-gray-700">
        ...
        >
          <select
            ...
            name="dropdown"
            id="dropdown"
            className="select text-white bg-red-500  select-bordered w-full"
          >
          ...
          <p className="px-6 text-sm text-center text-red-400">
            ...
            <Link to={"/login"} href="#" className="hover:underline text-white">
              Log in
            </Link>
            .
          </p>
